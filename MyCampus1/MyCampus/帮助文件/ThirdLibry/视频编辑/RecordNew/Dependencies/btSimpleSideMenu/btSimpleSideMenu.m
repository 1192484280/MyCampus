...
-(void)show
{
    if(!isOpen)
    {
        [UIView animateWithDuration:0.5 animations:^{
            ...
            menuTable.frame = CGRectMake(menuTable.frame.origin.x, 0, width, height);
            menuTable.alpha = 1;
...
-(void)hide
{
    if(isOpen)
    {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(hidSliderMenu)]) {
            
            [self.delegate hidSliderMenu];
        }
    }
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(cell == nil){
        ...
        circleView = [[UIView alloc]initWithFrame:CGRectMake(20, 20, 55, 55)];
...
-(void)commonInit:(UIViewController *)sender{
    ...
    if(!sender.navigationController.navigationBarHidden) {
        menuTable = [[UITableView alloc]initWithFrame:CGRectMake(xAxis, 0, width, height) style:UITableViewStyleGrouped];
    }else {
        menuTable = [[UITableView alloc]initWithFrame:CGRectMake(xAxis, 0, width, height) style:UITableViewStyleGrouped];
    }
    ...
    menuTable.backgroundColor = [[UIColor blackColor]colorWithAlphaComponent:0.2];
...
//    gesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:nil];
//    gesture.numberOfTapsRequired = 2;
    
    leftSwipe = [[UISwipeGestureRecognizer alloc]initWithTarget:self action:@selector(hide)];
    leftSwipe.direction = UISwipeGestureRecognizerDirectionLeft;
    
//    rightSwipe = [[UISwipeGestureRecognizer alloc]initWithTarget:self action:nil];
