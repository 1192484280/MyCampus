...
@property (nonatomic, strong) JFLocation *locaManager;
        ...
        dispatch_async(serialQueue, ^{
            [self processData:^(id success) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [_rootTableView reloadData];
                    self.locaManager = [[JFLocation alloc] init];
...
- (void)backBarButtonItem {
    UIButton *leftButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 55, 40)];
...
- (JFSearchView *)searchView {
    if (!_searchView) {
        ...
        _searchView = [[JFSearchView alloc] initWithFrame:CGRectMake(0, (NAVBAR_HEIGHT) + 41, frame.size.width, frame.size.height  - (NAVBAR_HEIGHT) - 41)];
