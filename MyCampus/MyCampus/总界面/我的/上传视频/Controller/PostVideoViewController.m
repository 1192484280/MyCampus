...
//    CGFloat navHeight = 0; //CGRectGetHeight(self.navigationController.navigationBar.bounds);
    ...
    _videoContentView.frame =  CGRectMake(CGRectGetMidX(self.view.frame) - sizeVideo.width/2, CGRectGetMidY(self.view.frame) - sizeVideo.height/2 - (iPhoneX_Top), sizeVideo.width, sizeVideo.height);
...
- (void)createContentView:(UIView *)parentView
{
    ...
    _captureContentView.centerY = (ScreenHeight - (iPhoneX_Top))/2;
    ...
    UIImage *addFileImage = [UIImage imageNamed:@"icon_videoUP"];
...
- (void)createVideoRangeSlider:(NSURL *)videoUrl
{
    ...
    _videoRangeSlider = [[SAVideoRangeSlider alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 50) videoUrl:videoUrl];
...
- (void)viewDidLoad
{
    ...
    im.image = [UIImage imageNamed:@"upVideo_02"];
...
- (UIView *)preView{
    ...
    if (!_preView) {
        ...
        _preView = [[PreView alloc] initWithFrame:CGRectMake(30, - 230, ScreenWidth -60, 230)];
...
- (void)setNaviBar
{
    ...
    UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 48, 30)];
    [btn setImage:[UIImage imageNamed:@"icon_next_normal"] forState:UIControlStateNormal];
    [btn addTarget:self action:@selector(handleConvert) forControlEvents:UIControlEventTouchUpInside];
...
- (void)handleConvert
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        self.preView.centerY = self.view.centerY - (iPhoneX_Top);
...
- (void)initToolbarView
{
    ...
    self.bottomControlView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 200, ScreenWidth, 50)];
    ...
    self.bottomView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, ScreenHeight - 40-(iPhoneX_Top), ScreenWidth, 40)];
