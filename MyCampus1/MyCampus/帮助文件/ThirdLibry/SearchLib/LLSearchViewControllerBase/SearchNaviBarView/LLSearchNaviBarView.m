...
- (instancetype)init{
    ...
    if (self = [super init]) {
        ...
        self.frame = CGRectMake(0, 0, ScreenWidth, NAVBAR_HEIGHT);
...
- (UIButton *)backBtn
{
    if (!_backBtn) {
        //左按钮
        _backBtn = ({
            UIButton *backBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, STATUS_BAR_HEIGHT, leftRightIconWH, leftRightIconWH)];
...
- (UIButton *)rightOneBtn
{
    if (!_rightOneBtn) {
        ...
        _rightOneBtn = ({
            UIButton *rightOneBtn = [[UIButton alloc] initWithFrame:CGRectMake(ScreenWidth-leftRightIconWH, STATUS_BAR_HEIGHT, leftRightIconWH, leftRightIconWH)];
...
- (UIView *)setupSearView
{
    ...
    self.searchBgView = ({
        UIView *searchBgView = [[UIView alloc] initWithFrame:CGRectMake(LeadMargin, STATUS_BAR_HEIGHT+(44-searchBgViewH)*0.5, ScreenWidth-TrailMargin-LeadMargin, searchBgViewH)];
...
- (void)modifySearchBarViewFrame
{
    if (!_backBtn.hidden && !_rightOneBtn.hidden) {
        ...
        self.searchBgView.width = ScreenWidth -self.backBtn.width-self.rightOneBtn.width;
    }
    else if (_backBtn.hidden && !_rightOneBtn.hidden)
    {
        self.searchBgView.x = LeadMargin;
        self.searchBgView.width = ScreenWidth -LeadMargin-self.rightOneBtn.width;
    }
    else if (!_backBtn.hidden && _rightOneBtn.hidden)
    {
        self.searchBgView.x = self.backBtn.width;
        self.searchBgView.width = ScreenWidth -TrailMargin-self.backBtn.width;
    }
