...
@interface ExportEffects ()<CAAnimationDelegate>
{
}
...
- (void)addAsset:(AVAsset *)asset toComposition:(AVMutableComposition *)composition withTrackID:(CMPersistentTrackID)trackID withRecordAudio:(BOOL)recordAudio withAssetFilePath:(NSString *)identifier
{
    ...
    if (recordAudio)
    {
        ...
        if ([[asset tracksWithMediaType:AVMediaTypeAudio] count] > 0)
        {
            ...
            if (self.musicFlag.length>0) {
                AVURLAsset* audioAsset =[[AVURLAsset alloc] initWithURL:[NSURL fileURLWithPath:self.musicFlag] options:nil];
                CMTimeRange audio_timeRange =CMTimeRangeMake(kCMTimeZero,asset.duration);//声音长度截取范围==视频长度
                AVMutableCompositionTrack *b_compositionAudioTrack = [composition addMutableTrackWithMediaType:AVMediaTypeAudio preferredTrackID:kCMPersistentTrackID_Invalid];
                [b_compositionAudioTrack insertTimeRange:audio_timeRange ofTrack:[[audioAsset tracksWithMediaType:AVMediaTypeAudio] objectAtIndex:0] atTime:totalDuration error:nil];
            }else{
...
- (void)addEffectToVideo:(NSString *)videoFilePath withAudioFilePath:(NSString *)audioFilePath withAniBeginTime:(CFTimeInterval)beginTime
{
    ...
    else
    {
        ...
        if (self.finishVideoBlock)
        {
            self.finishVideoBlock(NO, @"添加视频过程出错,请重新添加");
        }
    ...
    //文字
    CALayer *overlayLayer4 = [CALayer layer];
    UIImage *overlay4Image = nil;
    overlay4Image = self.image;
    
    [overlayLayer4 setContents:(id)[overlay4Image CGImage]];
    overlayLayer4.frame = parentLayer.bounds;
    
    [overlayLayer4 setMasksToBounds:YES];
    ...
    [_exportSession exportAsynchronouslyWithCompletionHandler:^(void) {
        switch ([_exportSession status])
        {
            case AVAssetExportSessionStatusCompleted:
            {
                ...
                if ([self.flag isEqualToString:@"上传"]) {
                    // Output path
                    self.filenameBlock = ^(void) {
                        return exportPath;
                    };
                    ...
                    if (self.finishVideoBlock)
                    {
                        self.finishVideoBlock(YES, @"处理完成");
                    }
