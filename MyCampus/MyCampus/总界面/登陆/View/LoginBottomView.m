...
- (UIButton *)weiboButton
{
    if (!_weiboButton) {
        ...
        [_weiboButton setImage:[UIImage imageNamed:@"icon_wechat"] forState:UIControlStateNormal];
    }
    return _weiboButton;
}
...
- (UIButton *)doubanButton
{
    if (!_doubanButton) {
        ...
        [_doubanButton setImage:[UIImage imageNamed:@"icon_wechat"] forState:UIControlStateNormal];
    }
...
- (UIButton *)peopleButton
{
    if (!_peopleButton) {
        ...
        [_peopleButton setImage:[UIImage imageNamed:@"icon_qq"] forState:UIControlStateNormal];
    }
...
- (UILabel *)titleLabel
{
    if (!_titleLabel) {
        ...
        _titleLabel.text = [NSString stringWithFormat:@"合作伙伴登录%@",[infoDictionary objectForKey:@"CFBundleDisplayName"] ];
        _titleLabel.textAlignment = NSTextAlignmentCenter;
        _titleLabel.textColor = [UIColor blackColor];
        _titleLabel.backgroundColor = [UIColor whiteColor];
        _titleLabel.font = [UIFont fontWithName:@"MFLiHei_Noncommercial-Regular" size:12];
    }
    return _titleLabel;
}
